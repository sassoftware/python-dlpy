...
class TestGPU(unittest.TestCase):
    ...
    def test_gpu03(self):
        ...
        # useTensorRT=True does not support multi-GPU in Viya 4.0.1.
        # Alireza is working on a fix to print out the warning message when there are multiple
        # GPU devices available AND no devices= specification. As of 05/21/2020, mva-vbviya
        # dev20004, model1.predict(data=data, gpu=Gpu(use_tensor_rt=True)) would fail. Specify
        # devices=[0] to enforce engaging one GPU for now.
    ...
    # Skip the test until Alireza fixes TensorRT issues.
    def skipped_test_gpu07(self):
        '''
        ...
        # Alireza is working on a fix to print out the warning message when there are multiple
        # GPU devices available AND no devices= specification. As of 05/21/2020, mva-vbviya
        # dev20004, model1.predict(data=data, gpu=Gpu(use_tensor_rt=True)) would fail. With
        # Alireza's fix, the command will complete and print out a warning message.
        # use_tensor_rt=True and precision='FP16' fall through the crack of below sections of
        # code in cudatrain.cu.
        # 112     if(dlP->gpuMixedPrecision && !dlP->gpuUseTensorRT)
        # 113         return TrainMain<float,__half>(ptr, parms);
        # 114     else
        # 115         return TrainMain<float,float>(ptr, parms);
        #
        # 1880                     interimBuff = (dlP->gpuMixedPrecision && !dlP->gpuUseTensorRT)?(void*)ccx->ysBuf[i][k][layerId]:(void*)ccx->ys[i][k][layerId];
        #
        # 2879                             if(dlP->gpuMixedPrecision){
        # 2880                                 status.cudaError = halfVec2float(
        # 2881                                                         ccx->ysBuf[parms->deviceId][parms->streamId][layerId],
        # 2882                                                         nRecs * layerPrivatePtr->YBlobSize,
        # 2883                                                         ccx->ys[parms->deviceId][parms->streamId][layerId],
        # 2884                                                         stream);
        # 2885                                 DEBUGStatusString(status, __FILE__, __LINE__);
        # 2886                                 interimBuff=(void*)ccx->ysBuf[parms->deviceId][parms->streamId][layerId];
        # 2887                                 if (status != _statusOK) goto Finish;
        # 2888                             }
        # And lines in cudatrt.cpp
        # 137      if(hasFP16){
        # 138          ctx->builder->setFp16Mode(true);
        # 139      } else{
        # 140          dlP->gpuMixedPrecision = FALSE;
        # 141      }
        #
        # Alireza will investigate. For now, do not apply use_tensor_rt=True with precision='FP16' together.
        '''
        r2 = model1.predict(data=data, gpu=Gpu(devices=[0], use_tensor_rt=True, precision='FP16'))
        '''
        r2 = self.s.dlScore(table=data, 
            modelTable=model1.model_table,
            initWeights=model1.model_weights,
            _debug=dict(display='d10c364.na.sas.com:0', ranks='0', debugger='/u/xunlwu/bin/start-cugdb'), 
            gpu=Gpu(devices=[0], use_tensor_rt=True, precision='FP16'))
    ...
    def test_gpu08(self):
        ...
        # dev20004, model1.predict(data=data, gpu=Gpu(use_tensor_rt=True)) would fail. With
        # Alireza's fix, the command will complete and print out a warning message.
        r2 = model1.predict(data=data, gpu=Gpu(devices=[0], use_tensor_rt=True, use_exclusive=True))
        self.assertTrue(r2.severity == 0)
    ...
    def skipped_test_gpu10(self):
        ...
        # Check comments in test_model07().
        r2 = model1.predict(data=data, 
                gpu=Gpu(devices=[0], use_tensor_rt=True, precision='FP16', use_exclusive=True))
        '''
        r2 = self.s.dlScore(table=data, 
            modelTable=model1.model_table,
            initWeights=model1.model_weights,
